...
class _SfCalendarState extends State<SfCalendar> with SingleTickerProviderStateMixin {
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      if (topPosition <= yPosition && yPosition < topPosition + panelHeight + appointmentViewPadding + dividerHeight) {
        ...
        if ((!_isRTL && viewPadding >= position.dx) || (_isRTL && _minWidth - viewPadding < position.dx)) {
          ...
          /// Return calendar details while the [getCalendarDetailsAtOffset]
          /// position placed on view header in schedule view.
          return CalendarDetails(widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(currentAppointments, widget.dataSource) : currentAppointments, DateTime(currentDate.year, currentDate.month, currentDate.day),
          ...
          for (int k = 0; k < currentAppointments.length; k++) {
            ...
            if (currentYPosition <= yPosition && currentYPosition + currentAppointmentHeight > yPosition) {
              ...
              return CalendarDetails(widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(selectedAppointment, widget.dataSource) : selectedAppointment, DateTime(currentDate.year, currentDate.month, currentDate.day),
                  CalendarElement.appointment, null);
            }
      ...
      case CalendarView.month:
        {
          if (isSameDate(_currentDate, _controller.displayDate) ||
              (isDateWithInDateRange(_currentViewVisibleDates[0], _currentViewVisibleDates[_currentViewVisibleDates.length - 1], _controller.displayDate) &&
                  (widget.monthViewSettings.numberOfWeeksInView != 6 || (widget.monthViewSettings.numberOfWeeksInView == 6 && _controller.displayDate!.month == _currentViewVisibleDates[_currentViewVisibleDates.length ~/ 2].month)))) {
            _currentDate = _controller.displayDate!;
            return;
          }
      ...
      widgets.add(MouseRegion(
          ...
          child: GestureDetector(
            child: _ScheduleAppointmentView(
                header: CustomPaint(
                    painter:
                        _AgendaDateTimePainter(currentDate, null, widget.scheduleViewSettings, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _locale, _calendarTheme, _themeData, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform),
                ...
                content: Container(
                  ...
                  child: AgendaViewLayout(null, widget.scheduleViewSettings, currentDate, currentAppointments, isRTL, _locale, _localizations, _calendarTheme, _themeData, _agendaViewNotifier, widget.appointmentTimeTextFormat, viewPadding, _textScaleFactor, _isMobilePlatform,
                      widget.appointmentBuilder, _minWidth - viewPadding, panelHeight, widget.monthViewSettings.agendaStyle.placeholderTextStyle, widget),
                )),
          ...
          child: _ScheduleAppointmentView(
              header: CustomPaint(
                  painter: _AgendaDateTimePainter(
                      currentDisplayDate, null, widget.scheduleViewSettings, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _locale, _calendarTheme, _themeData, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform),
              ...
              content: Container(
                ...
                child: CustomPaint(
                    painter: _ScheduleLabelPainter(currentDisplayDate, null, widget.scheduleViewSettings, false, isRTL, _locale, _useMobilePlatformUI, _agendaViewNotifier, _calendarTheme, _themeData, _localizations, _textScaleFactor, isDisplayDate: true),
  ...
  void _raiseCallbackForScheduleView(DateTime currentDate, Offset offset, List<CalendarAppointment> appointments, double viewHeaderWidth, double padding, bool isTapCallback, {bool isDisplayDate = false}) {
    ...
    if ((!_isRTL && viewHeaderWidth >= offset.dx) || (_isRTL && _minWidth - viewHeaderWidth < offset.dx)) {
      ...
      if (isTapCallback) {
        CalendarViewHelper.raiseCalendarTapCallback(widget, DateTime(currentDate.year, currentDate.month, currentDate.day),
            widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(currentAppointments, widget.dataSource) : currentAppointments, CalendarElement.viewHeader, null);
      } else {
        CalendarViewHelper.raiseCalendarLongPressCallback(widget, DateTime(currentDate.year, currentDate.month, currentDate.day),
            widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(currentAppointments, widget.dataSource) : currentAppointments, CalendarElement.viewHeader, null);
      }
    } else {
      /// Calculate the touch position appointment from its collection.
      ...
      for (int k = 0; k < appointments.length; k++) {
        ...
        if (currentYPosition <= offset.dy && currentYPosition + currentAppointmentHeight > offset.dy) {
          ...
          if (isTapCallback) {
            CalendarViewHelper.raiseCalendarTapCallback(widget, DateTime(currentDate.year, currentDate.month, currentDate.day),
                widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(selectedAppointment, widget.dataSource) : selectedAppointment, CalendarElement.appointment, null);
          } else {
            CalendarViewHelper.raiseCalendarLongPressCallback(widget, DateTime(currentDate.year, currentDate.month, currentDate.day),
                widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(selectedAppointment, widget.dataSource) : selectedAppointment, CalendarElement.appointment, null);
          }
                  ...
                  child: GestureDetector(
                    child: ScrollConfiguration(
                      ...
                      child: ListView(padding: EdgeInsets.zero, physics: const ClampingScrollPhysics(), controller: _resourcePanelScrollController, children: <Widget>[
                        ResourceViewWidget(_resourceCollection, widget.resourceViewSettings, resourceItemHeight, widget.cellBorderColor, _calendarTheme, _themeData, _resourceImageNotifier, isRTL, _textScaleFactor, _resourceHoverNotifier.value, _imagePainterCollection, resourceViewSize, panelHeight,
      ...
      return Positioned(
          ...
          child: _OpacityWidget(
              ...
              child: Container(
                  ...
                  child: GestureDetector(
                    child: AgendaViewLayout(widget.monthViewSettings, null, currentSelectedDate, null, isRTL, _locale, _localizations, _calendarTheme, _themeData, _agendaViewNotifier, widget.appointmentTimeTextFormat, 0, _textScaleFactor, _isMobilePlatform, widget.appointmentBuilder, width, height,
                        widget.monthViewSettings.agendaStyle.placeholderTextStyle, widget),
                    onTapUp: (TapUpDetails details) {
                ...
                child: MouseRegion(
                    ...
                    child: GestureDetector(
                      child: Stack(children: <Widget>[
                        CustomPaint(
                          painter: _AgendaDateTimePainter(
                              currentSelectedDate, widget.monthViewSettings, null, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _locale, _calendarTheme, _themeData, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform),
                        ...
                        Positioned(
                          ...
                          child: ListView(
                            ...
                            controller: _agendaScrollController,
                            children: <Widget>[
                              AgendaViewLayout(widget.monthViewSettings, null, currentSelectedDate, agendaAppointments, isRTL, _locale, _localizations, _calendarTheme, _themeData, _agendaViewNotifier, widget.appointmentTimeTextFormat, _agendaDateViewWidth, _textScaleFactor, _isMobilePlatform,
                                  widget.appointmentBuilder, width - _agendaDateViewWidth, painterHeight, widget.monthViewSettings.agendaStyle.placeholderTextStyle, widget),
                            ],
                          ),
                        ),
                      ]),
...
class _CalendarHeaderViewState extends State<_CalendarHeaderView> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color arrowColor = headerTextColor.withValues(alpha: headerTextColor.a * 0.6);
            ...
            child: Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                      clipBehavior: Clip.antiAlias,
                      decoration: BoxDecoration(
                        color: widget.showDatePickerButton && widget.isPickerShown ? Colors.grey.withValues(alpha: 0.3) : headerBackgroundColor,
                      ),
  ...
  Widget _getCalendarViewWidget(Color headerBackgroundColor, bool useMobilePlatformUI, bool isNeedIcon, double width, double height, TextStyle style, Color arrowColor, Color headerTextColor, CalendarView view, bool isHighlighted, Color? highlightColor, double defaultCalendarViewTextSize,
...
class _ScheduleLabelPainter extends CustomPainter {
  _ScheduleLabelPainter(this.startDate, this.endDate, this.scheduleViewSettings, this.isMonthLabel, this.isRTL, this.locale, this.useMobilePlatformUI, this.agendaViewNotifier, this.calendarTheme, this.themeData, this._localizations, this.textScaleFactor, {this.isDisplayDate = false})
  ...
  void _addDisplayDateLabel(Canvas canvas, Size size) {
    ...
    if (isDisplayDate && agendaViewNotifier.value != null && isSameDate(agendaViewNotifier.value!.hoveringDate, startDate)) {
      ...
      if (useMobilePlatformUI) {
        ...
        _backgroundPainter.color = calendarTheme.selectionBorderColor!.withValues(alpha: 0.4);
...
class _AgendaDateTimePainter extends CustomPainter {
  _AgendaDateTimePainter(this.selectedDate, this.monthViewSettings, this.scheduleViewSettings, this.todayHighlightColor, this.todayTextStyle, this.locale, this.calendarTheme, this.themeData, this.agendaDateNotifier, this.viewWidth, this.isRTL, this.textScaleFactor, this.isMobilePlatform)
  ...
  void _addDayLabelForMobile(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday, bool isMobile) {
    ...
    if (agendaDateNotifier.value != null && isSameDate(agendaDateNotifier.value!.hoveringDate, selectedDate)) {
      if (xPosition < agendaDateNotifier.value!.hoveringOffset.dx && xPosition + _textPainter.width > agendaDateNotifier.value!.hoveringOffset.dx && yPosition < agendaDateNotifier.value!.hoveringOffset.dy && yPosition + _textPainter.height > agendaDateNotifier.value!.hoveringOffset.dy) {
        _linePainter.color = isToday ? Colors.black.withValues(alpha: 0.1) : (themeData.brightness == Brightness.dark ? Colors.white : Colors.black87).withValues(alpha: 0.04);
        _drawTodayCircle(canvas, xPosition, yPosition, padding);
      }
    }
