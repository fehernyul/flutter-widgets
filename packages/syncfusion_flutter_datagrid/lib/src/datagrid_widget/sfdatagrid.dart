...
class SfDataGridState extends State<SfDataGrid> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _container = VisualContainerHelper(rowGenerator: _rowGenerator, dataGridStateDetails: dataGridStateDetails!);
  ...
  void _onDataGridThemeDataChanged(SfDataGridThemeData? newThemeData, SfColorScheme? newColorScheme) {
  ...
  void _setUp() {
    ...
    setStateDetailsInColumnSizer(_dataGridConfiguration.columnSizer, dataGridStateDetails!);

    //CurrentCell Manager initializing
    _dataGridConfiguration.currentCell = CurrentCellManager(dataGridStateDetails!);
    _dataGridConfiguration.dataGridFilterHelper = DataGridFilterHelper(dataGridStateDetails!);
    ...
    selection_manager.setStateDetailsInSelectionManagerBase(_rowSelectionManager!, dataGridStateDetails!);

    _controller!._addDataGridPropertyChangeListener(_handleSelectionPropertyChanged);

    _dataGridConfiguration.columnResizeController = ColumnResizeController(dataGridStateDetails: dataGridStateDetails!);
    _dataGridConfiguration.columnDragAndDropController = ColumnDragAndDropController(dataGridStateDetails: dataGridStateDetails!);
  ...
  void _initializeCellRendererCollection() {
    ...
    if (widget.onGetCellRenderers != null) {
      cellRenderers = widget.onGetCellRenderers!() as Map<String, GridCellRendererBase>;
      setStateDetailsInCellRendererBase(cellRenderers['TextField']!, dataGridStateDetails!);
      setStateDetailsInCellRendererBase(cellRenderers['ColumnHeader']!, dataGridStateDetails!);
      setStateDetailsInCellRendererBase(cellRenderers['StackedHeader']!, dataGridStateDetails!);
      setStateDetailsInCellRendererBase(cellRenderers['Checkbox']!, dataGridStateDetails!);
      setStateDetailsInCellRendererBase(cellRenderers['TableSummary']!, dataGridStateDetails!);
      setStateDetailsInCellRendererBase(cellRenderers['CaptionSummary']!, dataGridStateDetails!);
      setStateDetailsInCellRendererBase(cellRenderers['IndentCell']!, dataGridStateDetails!);
    } else {
  ...
  void _processCellUpdate(RowColumnIndex rowColumnIndex, {bool canRefreshGrouping = false}) {
    if (rowColumnIndex != RowColumnIndex(-1, -1)) {
      ...
      if (canRefreshGrouping && _dataGridConfiguration.source.groupedColumns.isNotEmpty) {
        _dataGridConfiguration.group!.clearDisplayElements(_dataGridConfiguration);
        updateDataSource(_dataGridConfiguration.source);
        notifyDataGridPropertyChangeListeners(dataGridStateDetails!().source, propertyName: 'grouping');
      }
    }
  }
  ...
  void _handleSelectionPropertyChanged({RowColumnIndex? rowColumnIndex, String? propertyName, bool recalculateRowHeight = false}) {
    selection_manager.handleSelectionPropertyChanged(dataGridConfiguration: dataGridStateDetails!(), propertyName: propertyName, rowColumnIndex: rowColumnIndex, recalculateRowHeight: recalculateRowHeight);
  }
    ...
    if (sortedColumns.length > 1) {
      for (final int i = 0; i < sortedColumns.length;) {
        ...
        if (compareResult != 0) {
          ...
          final List<SortColumnDetails> remainingSortColumns = sortedColumns.where((SortColumnDetails value) => value != sortColumn).toList(growable: false);
...
class DataGridThemeHelper {
  ...
  DataGridThemeHelper(SfDataGridThemeData dataGridThemeData, BuildContext context) {
    ...
    final _SfDataGridThemeData sfDataGridThemeData = _SfDataGridThemeData(context, dataGridThemeData);
    ...
    columnResizeIndicatorColor = defaults.columnResizeIndicatorColor ?? sfDataGridThemeData.columnResizeIndicatorColor;
    columnResizeIndicatorStrokeWidth = defaults.columnResizeIndicatorStrokeWidth ?? sfDataGridThemeData.columnResizeIndicatorStrokeWidth;
    ...
    filterIconHoverColor = defaults.filterIconHoverColor ?? sfDataGridThemeData.filterIconHoverColor;
    sortOrderNumberColor = defaults.sortOrderNumberColor ?? sfDataGridThemeData.sortOrderNumberColor;
    getSortOrderNumberWidget = defaults.getSortOrderNumberWidget ?? sfDataGridThemeData.getSortOrderNumberWidget;
    sortOrderNumberBackgroundColor = defaults.sortOrderNumberBackgroundColor ?? sfDataGridThemeData.sortOrderNumberBackgroundColor;
    ...
    filterPopupTextStyle = _filterPopupTextStyle!.merge(defaults.filterPopupTextStyle ?? sfDataGridThemeData.filterPopupTextStyle);
    filterPopupDisabledTextStyle = _filterPopupDisabledTextStyle!.merge(defaults.filterPopupDisabledTextStyle);

    columnDragIndicatorColor = defaults.columnDragIndicatorColor ?? sfDataGridThemeData.columnDragIndicatorColor;
    columnDragIndicatorStrokeWidth = defaults.columnDragIndicatorStrokeWidth ?? sfDataGridThemeData.columnDragIndicatorStrokeWidth;
...
class _SfDataGridThemeData extends SfDataGridThemeData {
  ...
  @override
  Color? get currentRowBackgoundColor => colorScheme.onSurface[19]?.withOpacity(0.2);

  @override
  DataGridCurrentCellStyle get currentCellStyle => DataGridCurrentCellStyle(borderColor: colorScheme.onSurface[66]!, borderWidth: 1.0, backgroundColor: Colors.transparent);
  ...
  @override
  TextStyle get rowHoverTextStyle => TextStyle(fontFamily: 'Roboto', fontWeight: FontWeight.w400, fontSize: 14, color: colorScheme.onSurface[222]);
  ...
  @override
  TextStyle get filterPopupTextStyle => TextStyle(fontSize: 14.0, color: colorScheme.onSurface[227], fontFamily: 'Roboto', fontWeight: FontWeight.normal);

  @override
  TextStyle get filterPopupDisabledTextStyle => TextStyle(fontSize: 14.0, color: colorScheme.onSurface[97], fontFamily: 'Roboto', fontWeight: FontWeight.normal);
