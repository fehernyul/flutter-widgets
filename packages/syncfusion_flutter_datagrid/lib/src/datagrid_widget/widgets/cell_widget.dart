...
class GridCell extends StatefulWidget {
  ...
  const GridCell({required Key key, required this.dataCell, required this.isDirty, required this.backgroundColor, required this.child, required this.dataGridStateDetails}) : super(key: key);
...
class _GridCellState extends State<GridCell> {
  ...
  bool _isDoubleTapEnabled(DataGridConfiguration dataGridConfiguration) => dataGridConfiguration.onCellDoubleTap != null || (dataGridConfiguration.allowEditing && dataGridConfiguration.editingGestureType == EditingGestureType.doubleTap);
  ...
  Future<void> _handleOnTapDown(TapDownDetails details, bool isSecondaryTapDown) async {
    ...
    if (widget.dataCell.cellType == CellType.stackedHeaderCell && dataGridConfiguration.currentCell.isEditing) {
      await dataGridConfiguration.currentCell.onCellSubmit(dataGridConfiguration);
    }

    if (_isDoubleTapEnabled(dataGridConfiguration)) {
      _handleDoubleTapOnEditing(dataGridConfiguration, dataCell, details, isSecondaryTapDown);
    }
  }

  void _handleDoubleTapOnEditing(DataGridConfiguration dataGridConfiguration, DataCellBase dataCell, TapDownDetails details, bool isSecondaryTapDown) {
    if (tapTimer != null && tapTimer!.isActive) {
      ...
      tapTimer = Timer(const Duration(milliseconds: 190), () {
        ...
        _handleOnTapUp(isSecondaryTapDown: isSecondaryTapDown, tapDownDetails: details, tapUpDetails: null, dataGridConfiguration: dataGridConfiguration, dataCell: dataCell, kind: _kind);
        tapTimer?.cancel();
      });
    }
  }
    ...
    return GestureDetector(
      onTapUp: isDoubleTapEnabled
          ...
          : (TapUpDetails details) {
              _handleOnTapUp(tapUpDetails: details, tapDownDetails: null, dataGridConfiguration: dataGridConfiguration, dataCell: dataCell, kind: _kind);
            },
      onTapDown: (TapDownDetails details) => _handleOnTapDown(details, false),
      onTap: isDoubleTapEnabled
          ? () {
              if (tapTimer != null && !tapTimer!.isActive) {
                _handleOnDoubleTap(dataCell: dataCell, dataGridConfiguration: dataGridConfiguration);
              }
            }
          : null,
      onTapCancel: () {
      ...
      onSecondaryTapUp: (TapUpDetails details) {
        _handleOnSecondaryTapUp(tapUpDetails: details, dataGridConfiguration: dataGridConfiguration, dataCell: dataCell, kind: _kind);
      },
...
class GridHeaderCell extends StatefulWidget {
  ...
  const GridHeaderCell({required Key key, required this.dataCell, required this.backgroundColor, required this.isDirty, required this.child, required this.dataGridStateDetails}) : super(key: key);
...
class _GridHeaderCellState extends State<GridHeaderCell> {
  ...
  void _handleOnTapUp(TapUpDetails tapUpDetails) {
    ...
    if (dataGridConfiguration.onCellTap != null) {
      final DataGridCellTapDetails details = DataGridCellTapDetails(rowColumnIndex: RowColumnIndex(dataCell.rowIndex, dataCell.columnIndex), column: dataCell.gridColumn!, globalPosition: tapUpDetails.globalPosition, localPosition: tapUpDetails.localPosition, kind: _kind);
  ...
  void _handleOnDoubleTap() {
    ...
    if (dataGridConfiguration.onCellDoubleTap != null) {
      final DataGridCellDoubleTapDetails details = DataGridCellDoubleTapDetails(rowColumnIndex: RowColumnIndex(dataCell.rowIndex, dataCell.columnIndex), column: dataCell.gridColumn!);
  ...
  Widget _wrapInsideContainer() {
    ...
    Widget child = _wrapInsideCellContainer(dataGridConfiguration: dataGridConfiguration, child: checkHeaderCellConstraints(widget.child), dataCell: widget.dataCell, key: widget.key!, backgroundColor: widget.backgroundColor);
    ...
    Widget getFeedbackWidget(DataGridConfiguration configuration) {
      return dataGridConfiguration.columnDragFeedbackBuilder != null
          ...
          : Container(
              ...
              decoration: BoxDecoration(color: dataGridConfiguration.dataGridThemeHelper!.feedBackWidgetColor, border: Border.all(color: dataGridConfiguration.dataGridThemeHelper!.gridLineColor!, width: dataGridConfiguration.dataGridThemeHelper!.gridLineStrokeWidth!)),
              child: widget.child);
    }
  ...
  void _ensureSortIconVisibility(GridColumn column, DataGridConfiguration? dataGridConfiguration) {
    if (dataGridConfiguration != null) {
      ...
      final SortColumnDetails? sortColumn = dataGridConfiguration.source.sortedColumns.firstWhereOrNull((SortColumnDetails sortColumn) => sortColumn.name == column.columnName);
  ...
  Widget _buildHeaderCell(Widget child, DataGridSortDirection? sortDirection, double availableWidth) {
    final DataGridConfiguration dataGridConfiguration = dataGridStateDetails();
    final GridColumn gridColumn = widget.dataCell.gridColumn!;
    final bool isSortedColumn = dataGridConfiguration.source.sortedColumns.any((SortColumnDetails element) => element.name == gridColumn.columnName);
    ...
    if (((hasSubOrder2 || isSortedColumn) || (gridColumn.allowSorting && dataGridConfiguration.allowSorting)) || (gridColumn.allowFiltering && dataGridConfiguration.allowFiltering)) {
      final double sortIconWidth = getSortIconWidth(dataGridConfiguration.columnSizer, gridColumn);
      final double filterIconWidth = getFilterIconWidth(dataGridConfiguration.columnSizer, gridColumn);
      ...
      if ((sortIconWidth > 0 && sortIconWidth < availableWidth) || (filterIconWidth > 0 && filterIconWidth < availableWidth)) {
        ...
        bool canShowColumnHeaderIcon() {
          final bool isFilteredColumn = dataGridConfiguration.source.filterConditions.containsKey(gridColumn.columnName);
          if (dataGridConfiguration.showColumnHeaderIconOnHover && dataGridConfiguration.isDesktop) {
            return isHovered || dataGridConfiguration.dataGridFilterHelper!.isFilterPopupMenuShowing || isFilteredColumn || isSortedColumn;
          } else {
            return true;
          }
        }
              ...
              child: isColumnHeaderIconVisible
                  ? Row(
                      children: <Widget>[
                        ...
                        if (children.containsKey('subFilterIcon')) children['subFilterIcon']!,
                      ],
                    )
                  : const SizedBox(),
            ),
        ...
        if (gridColumn.sortIconPosition == ColumnHeaderIconPosition.end && gridColumn.filterIconPosition == ColumnHeaderIconPosition.end) {
          headerCell = Row(
            children: <Widget>[Flexible(child: Container(child: child)), buildHeaderCellIcons(isColumnHeaderIconVisible)],
          );
        } else if (gridColumn.sortIconPosition == ColumnHeaderIconPosition.start && gridColumn.filterIconPosition == ColumnHeaderIconPosition.start) {
          headerCell = Row(
            children: <Widget>[
        ...
        } else if (gridColumn.sortIconPosition == ColumnHeaderIconPosition.end && gridColumn.filterIconPosition == ColumnHeaderIconPosition.start) {
          headerCell = Row(
            children: <Widget>[
  ...
  Widget _getSortNumber() {
    return Container(
      ...
      child: Center(
        child: Text(_sortNumber.toString(), style: TextStyle(fontSize: 12, color: _sortNumberTextColor)),
      ),
    );
  }
  ...
  void _sort(DataCellBase dataCell) {
    final DataGridConfiguration dataGridConfiguration = dataGridStateDetails();
    if (dataCell.dataRow?.rowType == RowType.headerRow && dataCell.dataRow?.rowIndex == grid_helper.getHeaderIndex(dataGridConfiguration)) {
      _makeSort(dataCell);
    }
  }
  ...
  Future<void> _makeSort(DataCellBase dataCell) async {
    ...
    if (dataGridConfiguration.currentCell.isEditing) {
      await dataGridConfiguration.currentCell.onCellSubmit(dataGridConfiguration, canRefresh: false);
    }
    ...
    if (column.allowSorting && dataGridConfiguration.allowSorting) {
      ...
      if (sortedColumns.isNotEmpty && allowMultiSort) {
        ...
        SortColumnDetails? sortedColumn = sortedColumns.firstWhereOrNull((SortColumnDetails sortColumn) => sortColumn.name == sortColumnName);
        if (sortedColumn == null) {
          ...
          if (sortedColumn.sortDirection == DataGridSortDirection.descending && dataGridConfiguration.allowTriStateSorting) {
            ...
            sortedColumn = SortColumnDetails(
              ...
              sortDirection: sortedColumn.sortDirection == DataGridSortDirection.ascending ? DataGridSortDirection.descending : DataGridSortDirection.ascending,
              sortIndex: sortedColumn.sortIndex,
            );
            final SortColumnDetails? removedSortColumn = sortedColumns.firstWhereOrNull((SortColumnDetails sortColumn) => sortColumn.name == sortedColumn!.name);
...
class _SortIconState extends State<_SortIcon> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _sortingAnimation = Tween<double>(begin: 0.0, end: pi).animate(CurvedAnimation(parent: _animationController, curve: Curves.easeIn));
  ...
  void didUpdateWidget(_SortIcon oldWidget) {
    ...
    if (oldWidget.sortDirection == DataGridSortDirection.ascending && widget.sortDirection == DataGridSortDirection.descending) {
      _animationController.forward();
    } else if (oldWidget.sortDirection == DataGridSortDirection.descending && widget.sortDirection == DataGridSortDirection.ascending) {
...
class _FilterIcon extends StatefulWidget {
  const _FilterIcon({Key? key, required this.column, required this.dataGridConfiguration}) : super(key: key);
  ...
  void onHandleTap(TapUpDetails details, BuildContext context) {
    if (widget.dataGridConfiguration.isDesktop) {
      ...
      showMenu(
          ...
          context: context,
          color: widget.dataGridConfiguration.dataGridThemeHelper!.filterPopupOuterColor,
          constraints: const BoxConstraints(maxWidth: 274.0),
          position: RelativeRect.fromSize(newOffset & Size.zero, viewSize),
          items: <PopupMenuEntry<String>>[
            _FilterPopupMenuItem<String>(column: widget.column, dataGridConfiguration: widget.dataGridConfiguration),
          ]).then((_) {
        if (widget.dataGridConfiguration.isDesktop) {
          notifyDataGridPropertyChangeListeners(widget.dataGridConfiguration.source, propertyName: 'Filtering');
          if (widget.dataGridConfiguration.source.groupedColumns.isNotEmpty) {
            notifyDataGridPropertyChangeListeners(widget.dataGridConfiguration.source, propertyName: 'grouping');
          }
          widget.dataGridConfiguration.dataGridFilterHelper!.isFilterPopupMenuShowing = false;
        }
      });
    } else {
      Navigator.push<_FilterPopup>(context, MaterialPageRoute<_FilterPopup>(builder: (BuildContext context) => _FilterPopup(column: widget.column, dataGridConfiguration: widget.dataGridConfiguration)));
    }
  }
        ...
        child: MouseRegion(
          ...
          child: isFiltered
              ? _FilteredIcon(
                  ...
                  iconColor: isHovered
                      ? (widget.dataGridConfiguration.dataGridThemeHelper!.filterIconHoverColor ?? widget.dataGridConfiguration.colorScheme!.onSurface.withOpacity(0.87))
                      : (widget.dataGridConfiguration.dataGridThemeHelper!.filterIconColor ?? widget.dataGridConfiguration.dataGridThemeHelper!.filterPopupIconColor!),
                  filterIcon: widget.dataGridConfiguration.dataGridThemeHelper!.filterIcon,
...
class _FilteredIcon extends StatelessWidget {
  const _FilteredIcon({Key? key, required this.iconColor, required this.filterIcon, required this.gridColumnName, required this.isSubFiltered}) : super(key: key);
            ...
            child: Column(
              ...
              children: <Widget>[
                Icon(
                  ...
                  const IconData(0xe704, fontFamily: 'FilterIcon', fontPackage: 'syncfusion_flutter_datagrid'),
                  size: 18.0,
                  color: iconColor,
...
class _FilterPopupMenuItem<T> extends PopupMenuItem<T> {
  const _FilterPopupMenuItem({required this.column, required this.dataGridConfiguration}) : super(child: null);
...
class _FilterPopup extends StatefulWidget {
  const _FilterPopup({Key? key, required this.column, required this.dataGridConfiguration}) : super(key: key);
...
class _FilterPopupState extends State<_FilterPopup> {
  ...
  void _initializeFilterProperties() {
    ...
    isAdvancedFilter = filterHelper.filterFrom == FilteredFrom.advancedFilter || widget.column.filterPopupMenuOptions?.filterMode == FilterMode.advancedFilterFirst || widget.column.filterPopupMenuOptions?.filterMode == FilterMode.advancedFilter;

    // === gabor - 2023.06.07
    isCustomFilter = widget.column.filterPopupMenuOptions?.filterMode == FilterMode.customFilter;
    ...
    final List<FilterCondition>? filterConditions = widget.dataGridConfiguration.source.filterConditions[widget.column.columnName];
    ...
    //andras - 2023.8.13
    advancedFilterHelper.neverUseDropDownFilterValues = widget.column.filterPopupMenuOptions!.neverUseDropDownFilterValues;
    ...
    if (filterConditions != null && isAdvancedFilter) {
      advancedFilterHelper.setAdvancedFilterValues(widget.dataGridConfiguration, filterConditions, filterHelper);
    } else {
  ...
  PreferredSize buildAppBar(BuildContext context) {
    return PreferredSize(
      ...
      child: AppBar(
        elevation: 0.0,
        bottom: PreferredSize(preferredSize: const Size.fromHeight(1.0), child: Container(height: 1.0, color: dataGridThemeHelper.filterPopupBorderColor)),
        backgroundColor: dataGridThemeHelper.filterPopupBackgroundColor,
        leading: IconButton(key: const ValueKey<String>('datagrid_filtering_cancelFilter_icon'), onPressed: closePage, icon: Icon(Icons.close, size: 22.0, color: dataGridThemeHelper.filterPopupIconColor)),
        centerTitle: false,
        titleSpacing: 0,
        title: Text(widget.dataGridConfiguration.localizations.sortAndFilterDataGridFilteringLabel, style: filterHelper.textStyle),
        actions: <Widget>[
          IconButton(
            key: const ValueKey<String>('datagrid_filtering_applyFilter_icon'),
            onPressed: canDisableOkButton() ? null : onHandleOkButtonTap,
            icon: Icon(Icons.check, size: 22.0, color: canDisableOkButton() ? widget.dataGridConfiguration.colorScheme!.onSurface.withOpacity(0.38) : filterHelper.primaryColor),
          ),
        ],
      ),
    );
  ...
  Widget _buildPopupView() {
    ...
    // === gabor - 2023.06.07
    bool isCustomFilterMenuEnabled = filterPopupMenuOptions.filterMode == FilterMode.customFilter;
    CustomGridFilterBaseWidget customFilterWidget = filterPopupMenuOptions.customFilterWidget ?? CustomGridFilterBaseWidget(doFilterRecords: () {}, doClearFilter: () {});
    ...
    bool isBothFilterEnabled = filterPopupMenuOptions.filterMode == FilterMode.both || filterPopupMenuOptions.filterMode == FilterMode.advancedFilterFirst;
    ...
    if (widget.column.filterPopupMenuOptions != null) {
      isCheckboxFilterEnabled = widget.column.filterPopupMenuOptions!.filterMode == FilterMode.checkboxFilter;
      isAdvancedFilterEnabled = widget.column.filterPopupMenuOptions!.filterMode == FilterMode.advancedFilter;
      isBothFilterEnabled = widget.column.filterPopupMenuOptions!.filterMode == FilterMode.both || widget.column.filterPopupMenuOptions!.filterMode == FilterMode.advancedFilterFirst;
      ...
      // === gabor - 2023.06.07
      isCustomFilterMenuEnabled = widget.column.filterPopupMenuOptions!.filterMode == FilterMode.customFilter;
      customFilterWidget = widget.column.filterPopupMenuOptions!.customFilterWidget ?? CustomGridFilterBaseWidget(doFilterRecords: () {}, doClearFilter: () {});
    }
            ...
            children: <Widget>[
              if (canShowSortingOptions)
                _FilterPopupMenuTile(
                    style: isSortAscendingEnabled ? filterHelper.textStyle : filterHelper.disableTextStyle,
                    height: filterHelper.tileHeight,
                    prefix: Icon(
                      const IconData(0xe700, fontFamily: 'FilterIcon', fontPackage: 'syncfusion_flutter_datagrid'),
                      // fontPackage: 'trendi_view_generator'),
                      color: isSortAscendingEnabled ? iconColor : dataGridThemeHelper.filterPopupDisableIconColor,
                      size: filterHelper.textStyle.fontSize! + 10,
                    ),
                    prefixPadding: EdgeInsets.only(left: 4.0, right: filterHelper.textStyle.fontSize!, bottom: filterHelper.textStyle.fontSize! > 14 ? filterHelper.textStyle.fontSize! - 14 : 0),
                    onTap: isSortAscendingEnabled ? onHandleSortAscendingTap : null,
                    child: Text(grid_helper.getSortButtonText(localizations, true, filterType), overflow: TextOverflow.ellipsis)),
              if (canShowSortingOptions)
                _FilterPopupMenuTile(
                  style: isSortDescendingEnabled ? filterHelper.textStyle : filterHelper.disableTextStyle,
                  height: filterHelper.tileHeight,
                  prefix: Icon(
                    const IconData(0xe701, fontFamily: 'FilterIcon', fontPackage: 'syncfusion_flutter_datagrid'),
                    // fontPackage: 'trendi_view_generator'),
                    color: isSortDescendingEnabled ? iconColor : dataGridThemeHelper.filterPopupDisableIconColor,
                    size: filterHelper.textStyle.fontSize! + 10,
                  ),
                  prefixPadding: EdgeInsets.only(left: 4.0, right: filterHelper.textStyle.fontSize!, bottom: filterHelper.textStyle.fontSize! > 14 ? filterHelper.textStyle.fontSize! - 14 : 0),
              ...
              if (canShowClearFilterOption)
                _FilterPopupMenuTile(
                  ...
                  height: filterHelper.tileHeight,
                  prefix: Icon(const IconData(0xe703, fontFamily: 'FilterIcon', fontPackage: 'syncfusion_flutter_datagrid'),
                      // fontPackage: 'trendi_view_generator'),
                      size: filterHelper.textStyle.fontSize! + 8,
                      color: isClearFilterEnabled ? iconColor : dataGridThemeHelper.filterPopupDisableIconColor),
                  prefixPadding: EdgeInsets.only(left: 4.0, right: filterHelper.textStyle.fontSize!, bottom: filterHelper.textStyle.fontSize! > 14 ? filterHelper.textStyle.fontSize! - 14 : 0),
                  onTap: isClearFilterEnabled ? onHandleClearFilterTap : null,
                  child: Text(getClearFilterText(localizations, showColumnName), overflow: TextOverflow.ellipsis),
                ),
              ...
              if (isBothFilterEnabled)
                _FilterPopupMenuTile(
                  ...
                  prefix: Icon(
                      ...
                      color: iconColor),
                  suffix: Icon(isAdvancedFilter ? Icons.keyboard_arrow_down : Icons.keyboard_arrow_right, size: filterHelper.textStyle.fontSize! + 6, color: iconColor),
                  prefixPadding: EdgeInsets.only(left: 4.0, right: filterHelper.textStyle.fontSize!, bottom: filterHelper.textStyle.fontSize! > 14 ? filterHelper.textStyle.fontSize! - 14 : 0),
                  child: Text(
              ...
              if (isCustomFilterMenuEnabled)
                _FilterPopupMenuTile(
                  ...
                  style: filterHelper.textStyle,
                  // height: filterHelper.tileHeight,
                  prefix: Icon(Icons.filter_alt, size: filterHelper.textStyle.fontSize! + 8, color: isClearFilterEnabled ? iconColor : dataGridThemeHelper.filterPopupDisableIconColor),
                  prefixPadding: EdgeInsets.only(left: 4.0, right: filterHelper.textStyle.fontSize!, bottom: filterHelper.textStyle.fontSize! > 14 ? filterHelper.textStyle.fontSize! - 14 : 0),
                  onTap: null,
                    ...
                    children: <Widget>[
                      SizedBox(
                        ...
                        child: ElevatedButton(
                            ...
                            onPressed: canDisableOkButton() ? null : onHandleOkButtonTap,
                            child: Text(localizations.okDataGridFilteringLabel, style: TextStyle(color: const Color(0xFFFFFFFF), fontSize: filterHelper.textStyle.fontSize, fontFamily: filterHelper.textStyle.fontFamily))),
                      ),
                      SizedBox(
                        width: 120.0,
                        height: filterHelper.tileHeight - 8,
                        child: OutlinedButton(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(color: filterHelper.primaryColor, fontSize: filterHelper.textStyle.fontSize, fontFamily: filterHelper.textStyle.fontFamily),
                            )),
                      ),
                    ],
                  ),
    ...
    if (isMobile) {
      return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) => buildPopup(viewSize: constraints.biggest));
    } else {
      return buildPopup();
    }
  }
  ...
  void onHandleOkButtonTap() {
    ...
    if (isCustomFilter) {
      widget.column.filterPopupMenuOptions?.customFilterWidget?.doFilterRecords();
    } else {
  ...
  bool hasFilterConditions() {
    return widget.dataGridConfiguration.source.filterConditions.containsKey(widget.column.columnName);
  }
  ...
  bool canDisableOkButton() {
    ...
    if (isAdvancedFilter) {
      ...
      return (helper.filterValue1 == null && helper.filterValue2 == null) && !helper.disableFilterTypes.contains(helper.filterType1) && !helper.disableFilterTypes.contains(helper.filterType2);
    } else {
      final bool? isSelectAllChecked = filterHelper.checkboxFilterHelper.isSelectAllChecked;
      return (isSelectAllChecked != null && !isSelectAllChecked) || filterHelper.checkboxFilterHelper.items.isEmpty;
    }
  }
  ...
  bool canEnableSortButton(DataGridSortDirection sortDirection) {
    ...
    if (configuration.allowSorting && widget.column.allowSorting) {
      return configuration.source.sortedColumns.isEmpty || !configuration.source.sortedColumns.any((SortColumnDetails column) => column.name == widget.column.columnName && column.sortDirection == sortDirection);
    }
    return false;
  }
...
class _FilterPopupMenuTile extends StatelessWidget {
  const _FilterPopupMenuTile({Key? key, required this.child, this.onTap, this.prefix, this.suffix, this.height, required this.style, this.prefixPadding = EdgeInsets.zero}) : super(key: key);
...
class _FilterMenuDropdown extends StatelessWidget {
  const _FilterMenuDropdown({required this.child, required this.padding, required this.height, this.suffix, Key? key}) : super(key: key);
...
class _CheckboxFilterMenu extends StatelessWidget {
  _CheckboxFilterMenu({Key? key, required this.setState, required this.column, required this.viewSize, required this.dataGridConfiguration}) : super(key: key);
    ...
    return Padding(
      ...
      child: Visibility(
        ...
        replacement: SizedBox(
          ...
          child: Center(child: Text(dataGridConfiguration.localizations.noMatchesDataGridFilteringLabel)),
        ),
        child: CheckboxTheme(
          data: CheckboxThemeData(
            side: BorderSide(width: 2.0, color: dataGridConfiguration.colorScheme!.onSurface.withOpacity(0.6)),
          ...
          child: Column(children: <Widget>[
            ...
            SizedBox(
              ...
              child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) => buildCheckboxTile(index, helper.textStyle)),
            ),
          ]),
        ),
      ),
      ...
      child: SizedBox(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
              enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: dataGridThemeHelper.filterPopupBorderColor!)),
              suffixIcon: Visibility(
                  ...
                  child: Icon(Icons.search, size: helper.textStyle.fontSize! + 8, color: dataGridThemeHelper.filterPopupIconColor)),
  ...
  Widget? buildCheckboxTile(int index, TextStyle style) {
    if (filterHelper.items.isNotEmpty) {
      ...
      final String displayText = dataGridConfiguration.dataGridFilterHelper!.getDisplayValue(element.value);
      return _FilterPopupMenuTile(
          style: style,
          height: isMobile ? style.fontSize! + 34 : style.fontSize! + 26,
          prefixPadding: const EdgeInsets.only(left: 4.0, right: 10.0),
          prefix: Checkbox(focusNode: checkboxFocusNode, value: element.isSelected, onChanged: (_) => onHandleCheckboxTap(element)),
  ...
  void onHandleSelectAllCheckboxTap() {
    final bool useSelected = filterHelper.isSelectAllInTriState || (filterHelper.isSelectAllChecked != null && filterHelper.isSelectAllChecked!);
...
class _AdvancedFilterPopupMenu extends StatelessWidget {
  const _AdvancedFilterPopupMenu({Key? key, required this.setState, required this.dataGridConfiguration, required this.advanceFilterTopPadding}) : super(key: key);
        ...
        children: <Widget>[
          _FilterMenuDropdown(
            ...
            child: Text(
              ...
              style: TextStyle(fontFamily: helper.textStyle.fontFamily, fontSize: helper.textStyle.fontSize, color: helper.textStyle.color, fontWeight: FontWeight.bold),
              overflow: TextOverflow.ellipsis,
            ),
          ),
          _FilterMenuDropdown(
      ...
      children: <Widget>[
        Row(children: <Widget>[
          SizedBox.fromSize(
            ...
            child: Radio<bool>(key: const ValueKey<String>('datagrid_filtering_and_button'), value: false, activeColor: helper.primaryColor, onChanged: handleChanged, groupValue: filterHelper.isOrPredicate),
          ),
        ...
        Row(children: <Widget>[
          SizedBox.fromSize(
            size: const Size(24.0, 24.0),
            child: Radio<bool>(key: const ValueKey<String>('datagrid_filtering_or_button'), value: true, activeColor: helper.primaryColor, onChanged: handleChanged, groupValue: filterHelper.isOrPredicate),
          ),
          const SizedBox(width: 8.0),
          Text(
  ...
  Widget _buildFilterValueDropdown({required bool isTopButton}) {
    ...
    Widget buildDropdownFormField() {
      return DropdownButtonHideUnderline(
        child: DropdownButtonFormField<Object>(
          ...
          key: isTopButton ? const ValueKey<String>('datagrid_filtering_filterValue_first_button') : const ValueKey<String>('datagrid_filtering_filterValue_second_button'),
          decoration: InputDecoration(
              enabledBorder: OutlineInputBorder(
                borderSide: BorderSide(color: dataGridThemeHelper.filterPopupBorderColor!),
              ),
              contentPadding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0),
              border: OutlineInputBorder(borderSide: BorderSide(color: dataGridThemeHelper.filterPopupBorderColor!))),
          icon: Icon(Icons.keyboard_arrow_down, size: helper.textStyle.fontSize! + 8, color: dataGridThemeHelper.filterPopupIconColor),
          isExpanded: true,
          value: isTopButton ? filterHelper.filterValue1 : filterHelper.filterValue2,
          style: helper.textStyle,
          items: filterHelper.items.map<DropdownMenuItem<Object>>((FilterElement value) => DropdownMenuItem<Object>(value: value.value, child: Text(helper.getDisplayValue(value.value)))).toList(),
    ...
    Widget buildTextField() {
      return TextField(
        style: helper.textStyle,
        key: isTopButton ? const ValueKey<String>('datagrid_filtering_filterValue_first_button') : const ValueKey<String>('datagrid_filtering_filterValue_second_button'),
        controller: isTopButton ? filterHelper.firstValueTextController : filterHelper.secondValueTextController,
        ...
        decoration: InputDecoration(
            enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: dataGridThemeHelper.filterPopupBorderColor!)), contentPadding: isMobile ? const EdgeInsets.all(16.0) : const EdgeInsets.all(8.0), border: const OutlineInputBorder(), hintStyle: const TextStyle(fontSize: 14.0)),
      );
    }

    return (filterHelper.neverUseDropDownFilterValues || canBuildTextField(isTopButton)) ? buildTextField() : buildDropdownFormField();
  }
  ...
  Widget _buildFilterTypeDropdown({required bool isFirstButton}) {
    ...
    void handleChanged(String? value) {
      ...
      if (filterHelper.textFieldFilterTypes.contains(value)) {
        if (isFirstButton) {
          filterHelper.firstValueTextController.text = helper.getDisplayValue(filterHelper.filterValue1);
        } else {
          filterHelper.secondValueTextController.text = helper.getDisplayValue(filterHelper.filterValue2);
        }
      } else {
        ...
        // dropdown.
        bool isInValidText(Object? filterValue) => !filterHelper.items.any((FilterElement element) => element.value == filterValue);
        if (isFirstButton) {
    ...
    return DropdownButtonHideUnderline(
      child: DropdownButtonFormField<String>(
        dropdownColor: dataGridThemeHelper.filterPopupOuterColor,
        key: isFirstButton ? const ValueKey<String>('datagrid_filtering_filterType_first_button') : const ValueKey<String>('datagrid_filtering_filterType_second_button'),
        decoration: InputDecoration(
            enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: dataGridThemeHelper.filterPopupBorderColor!)),
            contentPadding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0),
            border: OutlineInputBorder(borderSide: BorderSide(color: dataGridThemeHelper.filterPopupBorderColor!))),
        ...
        style: helper.textStyle,
        items: filterHelper.filterTypeItems.map<DropdownMenuItem<String>>((String value) => DropdownMenuItem<String>(value: value, child: Text(value))).toList(),
  ...
  Widget? _getTrailingWidget(BuildContext context, bool isFirstButton) {
    ...
    Future<void> handleDatePickerTap() async {
      ...
      final DateTime firstDate = filterHelper.items.first.value as DateTime; // TODO: ezt majd be kell allitani pl 1900.01.01-re
      final DateTime lastDate = filterHelper.items.last.value as DateTime; // TODO: ezt majd be kell allitani pl 1900.01.01-re
      DateTime initialDate = firstDate;

      if ((currentDate.isAfter(firstDate) && currentDate.isBefore(lastDate)) || (lastDate.day == currentDate.day && lastDate.month == currentDate.month && lastDate.year == currentDate.year)) {
      ...
      if (filterHelper.neverUseDropDownFilterValues) {
        ...
        final bool isVaildDate = filterHelper.items.any((FilterElement element) => element.value == selectedDate);
        final String? filterType = isFirstButton ? filterHelper.filterType1 : filterHelper.filterType2;
        final bool isValidType = filterType != null && filterHelper.textFieldFilterTypes.contains(filterType);
    ...
    Color getColor() {
      final bool isSelected = isFirstButton ? filterHelper.isCaseSensitive1 : filterHelper.isCaseSensitive2;
    ...
    if (filterHelper.advancedFilterType == AdvancedFilterType.text) {
      const IconData caseSensitiveIcon = IconData(0xe705, fontFamily: 'FilterIcon', fontPackage: 'syncfusion_flutter_datagrid');
      // const IconData caseSensitiveIcon = IconData(0xe705, fontFamily: 'FilterIcon', fontPackage: 'trendi_view_generator');

      return IconButton(
          key: isFirstButton ? const ValueKey<String>('datagrid_filtering_case_sensitive_first_button') : const ValueKey<String>('datagrid_filtering_case_sensitive_second_button'),
          iconSize: 22.0,
          splashRadius: 20.0,
          padding: EdgeInsets.zero,
          constraints: const BoxConstraints.tightFor(width: 22.0, height: 22.0),
      ...
      return IconButton(
          key: isFirstButton ? const ValueKey<String>('datagrid_filtering_date_picker_first_button') : const ValueKey<String>('datagrid_filtering_date_picker_second_button'),
          iconSize: 22.0,
          splashRadius: 20.0,
          padding: EdgeInsets.zero,
          constraints: const BoxConstraints.tightFor(width: 22.0, height: 22.0),
          onPressed: canEnableButton() ? handleDatePickerTap : null,
          icon: Icon(Icons.calendar_today_outlined, size: 22.0, color: dataGridConfiguration.colorScheme!.onSurface.withOpacity(0.6)));
    }
  }

  bool enableDropdownButton(bool isTopButton) {
    return !filterHelper.disableFilterTypes.contains(isTopButton ? filterHelper.filterType1 : filterHelper.filterType2);
  }
...
BorderDirectional _getCellBorder(DataGridConfiguration dataGridConfiguration, DataCellBase dataCell) {
  ...
  final int rowIndex = (dataCell.rowSpan > 0) ? dataCell.rowIndex - dataCell.rowSpan : dataCell.rowIndex;
  ...
  final bool isCaptionSummaryCoverdRow = dataCell.dataRow!.rowType == RowType.captionSummaryCoveredRow;
  ...
  final bool canSkipBottomBorder = grid_helper.getTableSummaryCount(dataGridConfiguration, GridTableSummaryRowPosition.bottom) > 0 && dataCell.rowIndex == grid_helper.getStartBottomSummaryRowIndex(dataGridConfiguration) - 1;

  // To draw the top border for the starting row of the bottom table summary row.
  final bool canDrawStartBottomSummaryRowTopBorder = isTableSummaryCell && dataCell.rowIndex == grid_helper.getStartBottomSummaryRowIndex(dataGridConfiguration);
  ...
  final bool canDrawHeaderHorizontalBorder =
      (dataGridConfiguration.headerGridLinesVisibility == GridLinesVisibility.horizontal || dataGridConfiguration.headerGridLinesVisibility == GridLinesVisibility.both) && (isHeaderCell || isStackedHeaderCell || (isIndentCell && (isHeaderRow || isStackedHeaderRow)));

  final bool canDrawHeaderVerticalBorder =
      (dataGridConfiguration.headerGridLinesVisibility == GridLinesVisibility.vertical || dataGridConfiguration.headerGridLinesVisibility == GridLinesVisibility.both) && (isHeaderCell || isStackedHeaderCell || (isIndentCell && (isHeaderRow || isStackedHeaderRow)));

  final ColumnDragAndDropController dragAndDropController = dataGridConfiguration.columnDragAndDropController;
  ...
  final bool canDrawLeftColumnDragAndDropIndicator = dataGridConfiguration.allowColumnsDragging &&
      ...
      (!dataGridConfiguration.showCheckboxColumn ? dragAndDropController.dragColumnStartIndex != dataCell.columnIndex : dragAndDropController.dragColumnStartIndex! + 1 != dataCell.columnIndex) &&
      isHeaderCell;
  ...
  final bool canSkipLeftColumnDragAndDropIndicator =
      canDrawLeftColumnDragAndDropIndicator && (!dataGridConfiguration.showCheckboxColumn ? dragAndDropController.dragColumnStartIndex! + 1 == dataCell.columnIndex : (dragAndDropController.dragColumnStartIndex! + 2 == dataCell.columnIndex || dragAndDropController.columnIndex == 0));

  final bool canSkipRightColumnDragAndDropIndicator =
      canDrawRightColumnDragAndDropIndicator && (!dataGridConfiguration.showCheckboxColumn ? dragAndDropController.dragColumnStartIndex! - 1 == dataCell.columnIndex : (dragAndDropController.dragColumnStartIndex! == dataCell.columnIndex || dragAndDropController.columnIndex == 0));

  final bool canDrawHorizontalBorder = (dataGridConfiguration.gridLinesVisibility == GridLinesVisibility.horizontal || dataGridConfiguration.gridLinesVisibility == GridLinesVisibility.both) && !isHeaderCell && !isStackedHeaderCell;

  final bool canDrawVerticalBorder = (dataGridConfiguration.gridLinesVisibility == GridLinesVisibility.vertical || dataGridConfiguration.gridLinesVisibility == GridLinesVisibility.both) && !isStackedHeaderCell && !isTableSummaryCell && !isHeaderCell;

  final GridColumn firstVisibleColumn = dataGridConfiguration.columns.firstWhere((GridColumn column) => column.visible && column.width != 0.0);

  final GridColumn? column = dataCell.gridColumn;

  // To draw the top outer border for the DataGrid.
  final bool canDrawGridTopOuterBorder = rowIndex == 0 && dataGridConfiguration.headerGridLinesVisibility != GridLinesVisibility.none;

  // To draw the left outer border for the indent cell of Headers.
  final bool canDrawHeaderIndentLeftOuterBorder = isGrouping && (isHeaderRow || isStackedHeaderRow) && columnIndex == 0 && dataGridConfiguration.headerGridLinesVisibility != GridLinesVisibility.none;

  // To draw the left outer border for the indent cell of DataGrid rows.
  final bool canDrawIndentLeftOuterBorder = isGrouping && (isDataRow || isCaptionSummaryCoverdRow) && columnIndex == 0 && dataGridConfiguration.gridLinesVisibility != GridLinesVisibility.none;

  // To draw the left outer border for the DataGrid rows with indentColumnWidth as zero.
  final bool canDrawGroupingRowsLeftOuterBoder =
      isGrouping && dataGridConfiguration.dataGridThemeHelper!.indentColumnWidth == 0 && ((isDataRow && column!.columnName == firstVisibleColumn.columnName) || isCaptionSummaryCoverdRow) && dataGridConfiguration.headerGridLinesVisibility != GridLinesVisibility.none;

  // To draw the left outer border for the Header with indentColumnWidth as zero.
  final bool canDrawGroupingHeaderLeftOuterBoder = isGrouping && dataGridConfiguration.dataGridThemeHelper!.indentColumnWidth == 0 && (isHeaderRow || isStackedHeaderRow) && column!.columnName == firstVisibleColumn.columnName && dataGridConfiguration.gridLinesVisibility != GridLinesVisibility.none;

  // To draw the left outer border for the DataGrid Headers.
  final bool canDrawGridHeaderLeftOuterBorder =
      ((isHeaderCell || isStackedHeaderCell) && dataGridConfiguration.headerGridLinesVisibility != GridLinesVisibility.none && (column!.columnName == firstVisibleColumn.columnName && !isGrouping)) || canDrawGroupingHeaderLeftOuterBoder || canDrawHeaderIndentLeftOuterBorder;

  // To draw the left outer border for the DataGrid Rows.
  final bool canDrawGridLeftOuterBorder =
      ((isRowCell || isTableSummaryCell || isCheckboxCell) && dataGridConfiguration.gridLinesVisibility != GridLinesVisibility.none && (column!.columnName == firstVisibleColumn.columnName && !isGrouping)) || canDrawGroupingRowsLeftOuterBoder || canDrawIndentLeftOuterBorder;

  // Frozen column and row checking
  final bool canDrawBottomFrozenBorder = dataGridConfiguration.frozenRowsCount.isFinite && dataGridConfiguration.frozenRowsCount > 0 && grid_helper.getLastFrozenRowIndex(dataGridConfiguration) == rowIndex;

  final bool canDrawTopFrozenBorder = dataGridConfiguration.footerFrozenRowsCount.isFinite && dataGridConfiguration.footerFrozenRowsCount > 0 && grid_helper.getStartFooterFrozenRowIndex(dataGridConfiguration) == rowIndex;

  final bool canDrawRightFrozenBorder = dataGridConfiguration.frozenColumnsCount.isFinite && dataGridConfiguration.frozenColumnsCount > 0 && grid_helper.getLastFrozenColumnIndex(dataGridConfiguration) == columnIndex;

  final bool canDrawLeftFrozenBorder = dataGridConfiguration.footerFrozenColumnsCount.isFinite && dataGridConfiguration.footerFrozenColumnsCount > 0 && grid_helper.getStartFooterFrozenColumnIndex(dataGridConfiguration) == columnIndex;

  final bool isFrozenPaneElevationApplied = dataGridConfiguration.dataGridThemeHelper!.frozenPaneElevation! > 0.0;

  final Color frozenPaneLineColor = dataGridConfiguration.dataGridThemeHelper!.frozenPaneLineColor!;

  final double frozenPaneLineWidth = dataGridConfiguration.dataGridThemeHelper!.frozenPaneLineWidth!;

  final bool canDrawIndentRightBorder =
      canDrawVerticalBorder && (dataGridConfiguration.source.groupedColumns.isNotEmpty && (columnIndex >= 0 && columnIndex < groupedColumnsLength && isIndentCell && columnIndex < dataCell.dataRow!.rowLevel - 1) || (isDataRow && isIndentCell && columnIndex < groupedColumnsLength));
  final Object? rowData = dataCell.dataRow!.rowData;

  final bool canDrawTableSummaryRowIndentBorder = (dataGridConfiguration.gridLinesVisibility == GridLinesVisibility.horizontal || dataGridConfiguration.gridLinesVisibility == GridLinesVisibility.both) && (isIndentCell && isTableSummaryRow);

  BorderSide getLeftBorder() {
    if ((columnIndex == 0 && (canDrawVerticalBorder || canDrawHeaderVerticalBorder || canDrawLeftColumnDragAndDropIndicator)) || canDrawLeftFrozenBorder || canDrawGridHeaderLeftOuterBorder || canDrawGridLeftOuterBorder) {
      if (canDrawLeftColumnDragAndDropIndicator && !canSkipLeftColumnDragAndDropIndicator) {
        return BorderSide(width: dataGridConfiguration.dataGridThemeHelper!.columnDragIndicatorStrokeWidth!, color: dataGridConfiguration.dataGridThemeHelper!.columnDragIndicatorColor!);
      }
      ...
      } else if ((columnIndex > 0 && ((canDrawVerticalBorder || canDrawHeaderVerticalBorder) && !canDrawLeftFrozenBorder)) || (canDrawGridLeftOuterBorder || canDrawGridHeaderLeftOuterBorder)) {
  ...
  BorderSide getTopBorder() {
    if ((rowIndex == 0 && (canDrawHorizontalBorder || canDrawHeaderHorizontalBorder)) || canDrawTopFrozenBorder || canDrawStartBottomSummaryRowTopBorder || canDrawGridTopOuterBorder) {
      if (canDrawTopFrozenBorder && !isStackedHeaderCell && !isFrozenPaneElevationApplied) {
        return BorderSide(width: frozenPaneLineWidth, color: frozenPaneLineColor);
      } else if ((canDrawHorizontalBorder && canDrawStartBottomSummaryRowTopBorder) || canDrawGridTopOuterBorder) {
        return BorderSide(width: borderWidth, color: borderColor);
      } else {
        return BorderSide.none;
      }
  ...
  BorderSide getRightBorder() {
    if (canDrawVerticalBorder || canDrawHeaderVerticalBorder || canDrawRightFrozenBorder || canDrawRightColumnDragAndDropIndicator || canDrawIndentRightBorder) {
      if (canDrawRightFrozenBorder && !isStackedHeaderCell && !isFrozenPaneElevationApplied) {
        return BorderSide(width: frozenPaneLineWidth, color: frozenPaneLineColor);
      } else if (canDrawRightColumnDragAndDropIndicator && !canSkipRightColumnDragAndDropIndicator) {
        return BorderSide(width: dataGridConfiguration.dataGridThemeHelper!.columnDragIndicatorStrokeWidth!, color: dataGridConfiguration.dataGridThemeHelper!.columnDragIndicatorColor!);
      } else if ((canDrawVerticalBorder || canDrawHeaderVerticalBorder) && !canDrawRightFrozenBorder && !isCaptionSummaryCell && !isIndentCell) {
        return BorderSide(width: borderWidth, color: borderColor);
      } else if ((canDrawIndentRightBorder || canDrawHeaderVerticalBorder || isCaptionSummaryCell) && !canDrawRightFrozenBorder) {
        return BorderSide(width: borderWidth, color: borderColor);
      } else {
        return BorderSide.none;
      }
  ...
  BorderSide getBottomBorder() {
    if (canDrawHorizontalBorder || canDrawHeaderHorizontalBorder || canDrawBottomFrozenBorder) {
      if (canDrawBottomFrozenBorder && !isStackedHeaderCell && !isFrozenPaneElevationApplied) {
        ...
        if (group is Group && isIndentCell && columnIndex >= group.level - 1 && rowIndex >= dataGridConfiguration.headerLineCount) {
          return BorderSide(width: borderWidth, color: borderColor);
        } else {
          return BorderSide.none;
        }
...
Widget _wrapInsideCellContainer({required DataGridConfiguration dataGridConfiguration, required DataCellBase dataCell, required Key key, required Color backgroundColor, required Widget child}) {
  final Color color = dataGridConfiguration.dataGridThemeHelper!.currentCellStyle!.borderColor;
  final double borderWidth = dataGridConfiguration.dataGridThemeHelper!.currentCellStyle!.borderWidth;
  ...
  double getCellHeight(DataCellBase dataCell, double defaultHeight) {
    ...
    if (dataCell.rowSpan > 0) {
      height = dataCell.dataRow!.getRowHeight(dataCell.rowIndex - dataCell.rowSpan, dataCell.rowIndex);
    } else {
  ...
  double getCellWidth(DataCellBase dataCell, double defaultWidth) {
    ...
    if (dataCell.columnSpan > 0) {
      width = dataCell.dataRow!.getColumnWidth(dataCell.columnIndex, dataCell.columnIndex + dataCell.columnSpan);
      if (dataGridConfiguration.source.groupedColumns.isNotEmpty && dataCell.dataRow!.rowType == RowType.tableSummaryCoveredRow) {
        width += dataGridConfiguration.dataGridThemeHelper!.indentColumnWidth * dataGridConfiguration.source.groupedColumns.length;
      }
    } else {
      width = defaultWidth;
    }
  ...
  Widget getChild(BoxConstraints constraint) {
    ...
    if (dataCell.isCurrentCell && dataCell.cellType != CellType.indentCell && dataCell.dataRow!.dataGridRow != null) {
      return Stack(
        children: <Widget>[
          ...
          Positioned(
              ...
              height: height,
              child: IgnorePointer(
                child: Container(key: key, clipBehavior: Clip.antiAlias, decoration: BoxDecoration(border: getBorder())),
              )),
        ],
      );
    } else {
...
bool _invokeGroupChangingCallback(DataGridConfiguration dataGridConfiguration, Group group) {
  final DataGridGroupChangingDetails details = DataGridGroupChangingDetails(key: group.key, groupLevel: group.level, isExpanded: group.isExpanded);
  if (group.isExpanded) {
...
void _invokeGroupChangedCallback(DataGridConfiguration dataGridConfiguration, Group group, bool isExpanded) {
  final DataGridGroupChangedDetails details = DataGridGroupChangedDetails(key: group.key, groupLevel: group.level, isExpanded: isExpanded);
...
Future<void> _handleOnTapUp({required TapUpDetails? tapUpDetails, required TapDownDetails? tapDownDetails, required DataCellBase dataCell, required DataGridConfiguration dataGridConfiguration, required PointerDeviceKind kind, bool isSecondaryTapDown = false}) async {
  ...
  if (!isSecondaryTapDown && dataGridConfiguration.onCellTap != null) {
    ...
    final DataGridCellTapDetails details = DataGridCellTapDetails(
        ...
        column: column,
        globalPosition: tapDownDetails != null ? tapDownDetails.globalPosition : tapUpDetails!.globalPosition,
        localPosition: tapDownDetails != null ? tapDownDetails.localPosition : tapUpDetails!.localPosition,
        kind: kind);
    dataGridConfiguration.onCellTap!(details);
  }
  ...
  if (dataGridConfiguration.source.groupedColumns.isNotEmpty && dataGridConfiguration.allowExpandCollapseGroup && dataCell.dataRow!.rowType == RowType.captionSummaryCoveredRow) {
...
Future<void> _handleOnDoubleTap({required DataCellBase dataCell, required DataGridConfiguration dataGridConfiguration}) async {
  // End edit the current editing cell if its editing mode is differed
  if (dataGridConfiguration.currentCell.isEditing) {
    if (await dataGridConfiguration.currentCell.canSubmitCell(dataGridConfiguration)) {
      await dataGridConfiguration.currentCell.onCellSubmit(dataGridConfiguration, cancelCanSubmitCell: true);
  ...
  if (dataGridConfiguration.onCellDoubleTap != null) {
    ...
    final DataGridCellDoubleTapDetails details = DataGridCellDoubleTapDetails(rowColumnIndex: RowColumnIndex(dataCell.rowIndex, dataCell.columnIndex), column: column);
    dataGridConfiguration.onCellDoubleTap!(details);
  }
...
Future<void> _handleOnSecondaryTapUp({required TapUpDetails tapUpDetails, required DataCellBase dataCell, required DataGridConfiguration dataGridConfiguration, required PointerDeviceKind kind}) async {
  ...
  if (dataGridConfiguration.onCellSecondaryTap != null) {
    ...
    final DataGridCellTapDetails details = DataGridCellTapDetails(rowColumnIndex: RowColumnIndex(dataCell.rowIndex, dataCell.columnIndex), column: column, globalPosition: tapUpDetails.globalPosition, localPosition: tapUpDetails.localPosition, kind: kind);
    dataGridConfiguration.onCellSecondaryTap!(details);
  }
}
